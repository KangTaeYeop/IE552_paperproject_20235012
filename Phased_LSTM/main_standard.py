...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='PyTorch Standard Sampling Example')
    ...
    # Training LSTM model with standard sampling
    print('Training LSTM with Standard Sampling.')
    model = Net(use_lstm=True).to(device)
    optimizer = optim.Adam(model.parameters(), lr=1e-3)

    # Test the initial model
    test(model, device, sin_wave_iterator(num_examples=1000, async_sample=False, sample_res=1.0))  # Standard sampling every 1 ms
    if args.save_model:
        torch.save(model.state_dict(), "lstm_standard_0.pt")  # Saving initial model for standard sampling

    lstm_test_acc = []
    for epoch in range(1, args.epochs + 1):
        train_data = sin_wave_iterator(async_sample=False, sample_res=1.0)  # Use standard sampling for training
        test_data = sin_wave_iterator(num_examples=1000, async_sample=False, sample_res=1.0)  # Use standard sampling for testing
        train(args, model, device, train_data, optimizer, epoch)
        test_acc = test(model, device, test_data)
        lstm_test_acc.append(test_acc)
        if args.save_model:
            torch.save(model.state_dict(), "lstm_standard.pt")  # Save model after each epoch

    # Training Phased LSTM model with standard sampling
    print('Training Phased LSTM with Standard Sampling.')
    ...
    test(model, device, sin_wave_iterator(num_examples=1000, async_sample=False, sample_res=1.0))  # Standard sampling every 1 ms
    if args.save_model:
        torch.save(model.state_dict(), "plstm_standard_0.pt")  # Saving initial model for standard sampling
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.save_model:
            torch.save(model.state_dict(), "plstm_standard.pt")  # Save model after each epoch

    pickle.dump({'lstm': lstm_test_acc, 'plstm': plstm_test_acc}, open("acc_standard.pkl", "wb"))
